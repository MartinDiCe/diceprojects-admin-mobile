...
class BrandDto {
  ...
  factory BrandDto.fromJson(Map<String, dynamic> json) => BrandDto(
        brandId: (json['brandId'] ?? json['id'])?.toString() ?? '',
...
class BrandsScreen extends ConsumerWidget {
  ...
  Widget _buildBody(
    ListState<BrandDto> state,
    BrandsNotifier notifier,
  ) {
    ...
    if (state.items.isEmpty) {
      return const EmptyState(
        ...
        title: 'Sin marcas',
        message: 'No hay marcas que coincidan con la búsqueda.',
      );
    }
      ...
      child: ListView.separated(
        ...
        itemBuilder: (ctx, i) {
          ...
          return AppEntityTile(
            ...
            details: [
              b.code,
              if ((b.description ?? '').trim().isNotEmpty) b.description!,
              if (b.isGlobal) 'Global',
            ],
            status: b.statusCode,
          );
        },
