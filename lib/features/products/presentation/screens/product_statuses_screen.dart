...
class _Notifier extends ListNotifier<_CatalogDto> {
  ...
  Future<PaginatedResponse<_CatalogDto>> fetchPage(PageParams params) async {
    final resp = await _dio.get('/v1/product-statuses', queryParameters: params.toQueryParams());
    return PaginatedResponse.fromJson(resp.data, _CatalogDto.fromJson);
  }
}

final _productstatusesscreenProvider = StateNotifierProvider.autoDispose<_Notifier, ListState<_CatalogDto>>(
  (ref) => _Notifier(ref.watch(dioProvider)),
);

class ProductStatusesScreen extends ConsumerWidget {
  const ProductStatusesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(_productstatusesscreenProvider);
    final notifier = ref.read(_productstatusesscreenProvider.notifier);
  ...
  Widget _buildBody(ListState<_CatalogDto> state, _Notifier notifier) {
    ...
    if (state.items.isEmpty) return EmptyState(icon: Icons.flag_rounded, title: 'Sin registros', message: 'No hay datos en este catálogo.');
