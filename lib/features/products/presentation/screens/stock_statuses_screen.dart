...
class _Notifier extends ListNotifier<_CatalogDto> {
  ...
  Future<PaginatedResponse<_CatalogDto>> fetchPage(PageParams params) async {
    final resp = await _dio.get('/v1/stock-statuses', queryParameters: params.toQueryParams());
    return PaginatedResponse.fromJson(resp.data, _CatalogDto.fromJson);
  }
}

final _stockstatusesscreenProvider = StateNotifierProvider.autoDispose<_Notifier, ListState<_CatalogDto>>(
  (ref) => _Notifier(ref.watch(dioProvider)),
);

class StockStatusesScreen extends ConsumerWidget {
  const StockStatusesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(_stockstatusesscreenProvider);
    final notifier = ref.read(_stockstatusesscreenProvider.notifier);
  ...
  Widget _buildBody(ListState<_CatalogDto> state, _Notifier notifier) {
    ...
    if (state.items.isEmpty) return EmptyState(icon: Icons.inventory_rounded, title: 'Sin registros', message: 'No hay datos en este catálogo.');
