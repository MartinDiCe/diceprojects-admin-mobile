...
class _Notifier extends ListNotifier<_CatalogDto> {
  ...
  Future<PaginatedResponse<_CatalogDto>> fetchPage(PageParams params) async {
    final resp = await _dio.get('/v1/price-types', queryParameters: params.toQueryParams());
    return PaginatedResponse.fromJson(resp.data, _CatalogDto.fromJson);
  }
}

final _pricetypesscreenProvider = StateNotifierProvider.autoDispose<_Notifier, ListState<_CatalogDto>>(
  (ref) => _Notifier(ref.watch(dioProvider)),
);

class PriceTypesScreen extends ConsumerWidget {
  const PriceTypesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(_pricetypesscreenProvider);
    final notifier = ref.read(_pricetypesscreenProvider.notifier);
  ...
  Widget _buildBody(ListState<_CatalogDto> state, _Notifier notifier) {
    ...
    if (state.items.isEmpty) return EmptyState(icon: Icons.price_change_rounded, title: 'Sin registros', message: 'No hay datos en este catálogo.');
